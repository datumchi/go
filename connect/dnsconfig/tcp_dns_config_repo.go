...
type NetworkAwareDNSConfigRepo struct {
...
func CreateNetworkAwareDNSConfigRepo() NetworkAwareDNSConfigRepo {
	configRepo := NetworkAwareDNSConfigRepo{
...
func (configRepo NetworkAwareDNSConfigRepo) GetDomainDefinition(domain string) (naming.DomainDefinition, error) {
	...
	for _, entry := range textEntries {
		...
		if strings.Contains(entry, "v=datumchi_pubkey") {
			err = configRepo.parsePublicKey(entry, &domainDefinition)
			if err != nil {
				return domainDefinition, err
			}
		} else if strings.Contains(entry, "v=datumchi_signature") {
			err = configRepo.parseSignature(entry, &domainDefinition)
			if err != nil {
				return domainDefinition, err
			}
		} else {
...
func (configRepo NetworkAwareDNSConfigRepo) parsePublicKey(entry string, domainDefinition *naming.DomainDefinition) error {

	submatches := configRepo.PublicKeyRegExp.FindStringSubmatch(entry)
	if len(submatches) != 2 {
...
func (configRepo NetworkAwareDNSConfigRepo) parseSignature(entry string, domainDefinition *naming.DomainDefinition) error {

	submatches := configRepo.SignatureRegExp.FindStringSubmatch(entry)
	if len(submatches) != 2 {
